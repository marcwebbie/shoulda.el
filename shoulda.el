  ...
  (save-excursion
    ...
    (let* ((name-regex "\\(\\(:[a-z0-9_]+\\)\\|\\([\"']\\([a-z0-9_ ]+\\)[\"']\\)\\)")
      ...
      (when (and should context)
        (ruby-compilation (concat "cd " (project-root) " && bundle exec -- ruby " (shell-quote-argument (buffer-file-name)) " -n /'"  should "'/"))))))
  ...
  (save-excursion
    ...
    (let* ((name-regex "\\(\\(:[a-z0-9_]+\\)\\|\\([\"']\\([a-z0-9_ ]+\\)[\"']\\)\\)")
           ...
           (context (when (search-backward-regexp (concat "[ \t]*context +" name-regex "[ \t]+do") nil t)
                      (funcall name-match))))
      (when (and should context)
        (ruby-compilation (concat "cd " (project-root) " && bundle exec -- ruby " (shell-quote-argument (buffer-file-name)) " -n /'"  context "'/"))))))
